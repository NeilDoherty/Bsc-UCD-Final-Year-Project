...
public class WelchHashMap
{
	...
	public static void main(String[] args)
	{
		...
		Map<Integer, Collection<Integer>> permutation = new HashMap<Integer, Collection<Integer>>();	
		...
		{	
			...
			{	
				...
				Collection<Integer> values = permutation.get(inc);
				if (values==null)
				{
				    values = new ArrayList<Integer>();
				    permutation.put(inc, values);
				}
				values.add(res1array[i]);
			}
			for (increment=0;increment<(N-1);increment++)
			{
				...
				values.add(res1array[increment]);
				inc++;
			}
		}
		final long endTime = System.currentTimeMillis();
	    double output = (endTime - startTime)/1000.0;
	    System.out.println("Exhaustive search took " + output + " seconds to find " + permutation.size() + " unique costas arrays for order N = " + N);
	}
}